...
def wilson_factorization(S, freq, fs, Niterations=100, tol=1e-12):
	m = S.shape[0]    
	N = freq.shape[0]-1
	...
	for i in range(m):
		for j in range(m):
			gam[i,j,:] = (np.fft.ifft(Sarr[i,j,:])*fs).real
...
def granger_causality(S, H, Z):
	...
	Ix2y  = np.zeros(N) * (1 + 1j)
	Iy2x  = np.zeros(N) * (1 + 1j)
	Ix2y = np.log( Syy/(Hyy_circf*Z[1,1]*np.conj(Hyy_circf)) )
	Iy2x = np.log( Sxx/(Hxx_tilda*Z[0,0]*np.conj(Hxx_tilda)) )

	Ixy  = np.zeros(N) * (1 + 1j)
	for i in range(N):
		Ixy[i]  = np.log( (Hxx_tilda[i]*Z[0,0]*np.conj(Hxx_tilda[i]))*(Hyy_circf[i]*Z[1,1]*np.conj(Hyy_circf[i])/np.linalg.det(S[:,:,i])) )
